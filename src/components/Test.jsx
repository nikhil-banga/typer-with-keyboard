...
const Test = () => {
  ...
  const [remainingTime, setRemainingTime] = useState(0); // Initialize remainingTime to 0
  ...
  }, [startTime, isCompleted, input, sentence]);
  ...
  const handleInputChange = (e) => {
    ...
    const accuracy = (1 - incorrectIndexes.length / sentence.length) * 100;
  ...
  const renderSentence = () => {
    ...
    return (
      <div className="my-4">
        ...
        <textarea
          className="border border-gray-300 px-4 py-2 m-2 rounded-lg w-full h-16 resize-none focus:outline-none focus:border-blue-500"
          value={input}
    ...
    return (
      <div className="flex justify-center space-x-4 my-4">
        <div>
          ...
          <p className="text-2xl text-white font-medium">{accuracy}%</p>
        </div>
        <div>
          <p className="text-white">Words Per Minute</p>
          <p className="text-2xl text-white font-medium">
            {isCompleted ? wordCountPerMinute : "-"}
          </p>
        </div>
        <div>
